import { __spreadProps, __spreadValues } from "./chunk-HSNDBVJ3.js"
...
function consumerAfterComputation(node, prevConsumer) {
	...
	if (consumerIsLive(node)) {
		for (
			let i = node.nextProducerIndex;
			i < node.producerNode.length;
			i++
		) {
			producerRemoveLiveConsumerAtIndex(
				node.producerNode[i],
...
function consumerIsLive(node) {
	return (
		node.consumerIsAlwaysLive || (node?.liveConsumerNode?.length ?? 0) > 0
	)
}
...
function signalSetFn(node, newValue) {
	...
	if (Object.is(value, newValue)) {
		if (
			typeof ngDevMode !== "undefined" &&
			ngDevMode &&
			!node.equal(value, newValue)
		) {
			console.warn(
				"Signal value equality implementations should always return `true` for values that are the same according to `Object.is` but returned `false` instead.",
			)
		}
...
function truncateMiddle(str, maxLength = 100) {
	...
	return (
		str.substring(0, halfLimit) +
		"..." +
		str.substring(str.length - halfLimit)
	)
}
...
function throwProviderNotFoundError(token, injectorName) {
	...
	throw new RuntimeError(
		-201,
		ngDevMode &&
			`No provider for ${stringifyForError(
				token,
			)} found${injectorDetails}`,
	)
}
...
function assertDomNode(node) {
	if (!(node instanceof Node)) {
		throwError2(
			`The provided value must be an instance of a DOM Node but got ${stringify(
				node,
			)}`,
		)
	}
		...
		if (typeof options == "number") {
			;(typeof ngDevMode === "undefined" || ngDevMode) &&
				assertLessThan(
					options,
					0,
					"Only negative numbers are supported here",
				)
...
function injectorProfiler(event) {
	!ngDevMode &&
		throwError2(
			"Injector profiler should never be called in production mode",
		)
...
;(function (ChangeDetectionStrategy2) {
	ChangeDetectionStrategy2[(ChangeDetectionStrategy2["OnPush"] = 0)] =
		"OnPush"
...
function classIndexOf(className, classToSearch, startingIndex) {
	ngDevMode &&
		assertNotEqual(classToSearch, "", 'can not look for "" string.')
	...
	while (i < attrs.length) {
		...
		if (typeof item === "string" && isImplicitAttrsSection) {
			...
			if (isProjectionMode && item === "class") {
				if (
					classIndexOf(value.toLowerCase(), cssClassToMatch, 0) !== -1
				) {
					return true
				}
			}
...
function ɵɵdefineComponent(componentDefinition) {
	return noSideEffects(() => {
		...
		const def = __spreadProps(__spreadValues({}, baseDef), {
			...
			pipeDefs: null,
			dependencies:
				(baseDef.standalone && componentDefinition.dependencies) ||
				null,
			...
			if (previousCompDefType !== componentDef.type) {
				console.warn(
					formatRuntimeError(
						...
						}' with selector '${stringifyCSSSelectorList(
							componentDef.selectors,
...
;(function (LContainerFlags2) {
	...
	LContainerFlags2[(LContainerFlags2["HasChildViewsToRefresh"] = 4)] =
...
function assertLViewOrUndefined(value) {
	value &&
		assertEqual(
			isLView(value),
			true,
			"Expecting LView or undefined or null",
		)
}
...
function getFactoryDef(type, throwNotFound) {
	...
	if (!hasFactoryDef && throwNotFound === true && ngDevMode) {
		throw new Error(
			`Type ${stringify(type)} does not have 'ɵfac' property.`,
		)
	}
...
function markAncestorsForTraversal(lView) {
	...
	while (parent !== null) {
		if (
			...
			(isLView(parent) && parent[FLAGS] & 8192)
		) {
			break
		}
...
function enterDI(lView, tNode, flags) {
	...
	if (flags & InjectFlags.SkipSelf) {
		...
		while (true) {
			ngDevMode &&
				assertDefined(parentTNode, "Parent TNode should be defined")
...
function enterView(newView) {
	...
	if (ngDevMode) {
		...
		assertEqual(
			newLFrame.currentDirectiveIndex,
			-1,
			"Expected clean LFrame",
		)
	...
	ngDevMode &&
		tView.firstChild &&
		assertTNodeForTView(tView.firstChild, tView)
...
function callHooks(currentView, arr, initPhase, currentNodeIndex) {
	...
	const startIndex =
		currentNodeIndex !== void 0
			? currentView[PREORDER_HOOK_FLAGS] & 65535
			: 0
...
function assertTNodeType(tNode, expectedTypes, message) {
	...
	if ((tNode.type & expectedTypes) === 0) {
		throwError2(
			message ||
				`Expected [${toTNodeTypeAsString(
					expectedTypes,
				)}] but got ${toTNodeTypeAsString(tNode.type)}.`,
		)
	}
}
...
function getCompilerFacade(request) {
	...
	if (typeof ngDevMode === "undefined" || ngDevMode) {
		console.error(
			`JIT compilation failed for ${request.kind}`,
			request.type,
		)
...
var ReflectionCapabilities = class {
	...
	_ownParameters(type, parentCtor) {
		...
		if (
			tsickleCtorParams &&
			tsickleCtorParams !== parentCtor.ctorParameters
		) {
	...
	ownPropMetadata(typeOrFunc) {
		...
		return (
			this._ownPropMetadata(typeOrFunc, getParentCtor(typeOrFunc)) || {}
		)
	}
	...
	if (Array.isArray(dep) && dep.length > 0) {
		for (let j = 0; j < dep.length; j++) {
			...
			if (
				...
				param instanceof Self ||
				proto.ngMetadataName === "Self"
			) {
				meta.self = true
			} else if (
				param instanceof Host ||
				proto.ngMetadataName === "Host"
			) {
				meta.host = true
	...
	get(
		...
		try {
			if (!(flags & InjectFlags.SkipSelf)) {
				...
				if (record === void 0) {
					const def =
						couldBeInjectableType(token) && getInjectableDef(token)
		...
		if (def != null) {
			...
			if (isModuleWithProviders(depType)) {
				...
				throw new Error(
					`The "${stringifyForError(
						depType,
					)}" type, imported from "${stringifyForError(
						importingType,
	...
	computeNgModuleScope(type) {
		...
		for (const imported of maybeUnwrapFn(def.imports)) {
			if (isNgModule(imported)) {
				...
				addSet(
					importedScope.exported.directives,
					scope.compilation.directives,
				)
		...
		for (const exported of maybeUnwrapFn(def.exports)) {
			if (isNgModule(exported)) {
				...
				addSet(
					exportedScope.exported.directives,
					scope.exported.directives,
				)
...
function validateElementIsKnown(
	...
	if (!hasDirectives && tagName !== null) {
		...
		if (isUnknown && !matchingSchemas(schemas, tagName)) {
			...
			const schemas2 = `'${
				isHostStandalone ? "@Component" : "@NgModule"
			}.schemas'`
...
function registerLView(lView) {
	ngDevMode &&
		assertNumber(
			lView[ID],
			"LView must have an ID in order to be registered",
		)
...
function applyProjection(tView, lView, tProjectionNode) {
	...
	let beforeNode = getInsertInFrontOfRNode(
		parentTNode,
		tProjectionNode,
		lView,
	)
...
function ɵɵvalidateIframeAttribute(attrValue, tagName, attrName) {
	...
	if (tNode.type === 2 && tagName.toLowerCase() === "iframe") {
		...
		const errorMessage =
			ngDevMode &&
			`Angular has detected that the \`${attrName}\` was applied as a binding to an <iframe>${getTemplateLocationDetails(
				lView,
			)}. For security reasons, the \`${attrName}\` can be set on an <iframe> as a static attribute only. 
...
function retrieveHydrationInfoImpl(rNode, injector, isRootView2 = false) {
	...
	const remainingNgh = isRootView2
		? componentViewNgh
		: rootViewNgh
		  ? `|${rootViewNgh}`
		  : ""
		...
		while (nextIt || nextRemove) {
			...
			if (record === nextRemove) {
				...
				if (record.previousIndex == null) {
					...
					const localMovePreviousIndex =
						adjPreviousIndex - addRemoveOffset
	...
	check(collection) {
		...
		if (Array.isArray(collection)) {
			...
			for (let index2 = 0; index2 < this.length; index2++) {
				...
				if (
					record === null ||
					!Object.is(record.trackById, itemTrackBy)
				) {
...
function devModeEqual(a, b) {
	...
	if (isListLikeIterableA && isListLikeIterableB) {
		...
		const isBObject =
			b && (typeof b === "object" || typeof b === "function")
		if (
			!isListLikeIterableA &&
			isAObject &&
			!isListLikeIterableB &&
...
function throwMultipleComponentError(tNode, first2, second) {
	throw new RuntimeError(
		...
		`Multiple components match node with tagname ${
			tNode.value
		}: ${stringifyForError(first2)} and ${stringifyForError(second)}`,
	)
}
function throwErrorIfNoChangesMode(
	...
	let msg = `ExpressionChangedAfterItHasBeenCheckedError: Expression has changed after it was checked. Previous value${field}: '${formatValue(
		...
		componentClassName
			? ` Expression location: ${componentClassName} component`
			: ""
	}`
	if (creationMode) {
...
function createLView(
	...
	lView[ENVIRONMENT] =
		environment || (parentLView && parentLView[ENVIRONMENT])
	ngDevMode &&
		assertDefined(lView[ENVIRONMENT], "LViewEnvironment is required")
	lView[RENDERER] = renderer || (parentLView && parentLView[RENDERER])
	ngDevMode && assertDefined(lView[RENDERER], "Renderer is required")
	lView[INJECTOR$1] =
		injector || (parentLView && parentLView[INJECTOR$1]) || null
...
function allocExpando(tView, lView, numSlotsToAlloc, initialValue) {
	...
	if (ngDevMode) {
		...
		assertSame(
			tView,
			lView[TVIEW],
			"`LView` must be associated with `TView`!",
		)
		assertEqual(
			tView.data.length,
			lView.length,
...
function createTView(
	...
	const consts =
		typeof constsOrFactory === "function"
			? constsOrFactory()
			: constsOrFactory
...
function createTNode(tView, tParent, type, index, value, attrs) {
	...
	ngDevMode &&
		assertNotSame(
			attrs,
			void 0,
			"'undefined' is not valid value for 'attrs'",
		)
...
function elementPropertyInternal(
	...
	if (!nativeOnly && inputData != null && (dataValue = inputData[propName])) {
		...
		if (ngDevMode) {
			...
			if (
				!isPropertyValid(element, propName, tNode.value, tView.schemas)
			) {
				handleUnknownPropertyError(
					propName,
					tNode.value,
					tNode.type,
					lView,
...
function writeToDirectiveInput(def, instance, publicName, privateName, value) {
	...
	try {
		...
		if (
			inputTransforms !== null &&
			inputTransforms.hasOwnProperty(privateName)
		) {
		...
		try {
			for (let i = 0; i < contentQueries.length; i += 2) {
				...
				if (directiveDefIdx !== -1) {
					...
					ngDevMode &&
						assertDefined(directiveDef, "DirectiveDef not found.")
					ngDevMode &&
						assertDefined(
...
function textBindingInternal(lView, index, value) {
	...
	ngDevMode &&
		assertNotSame(value, NO_CHANGE, "value should not be NO_CHANGE")
	ngDevMode && assertIndexInRange(lView, index)
	...
	if (getActiveConsumer() !== null) {
		throw new RuntimeError(
			...
			ngDevMode &&
				`${
					debugFn.name
				}() cannot be called from within a reactive context.${
...
var AsyncStackTaggingZoneSpec = class {
	constructor(namePrefix, consoleAsyncStackTaggingImpl = console) {
		...
		this.createTask =
			consoleAsyncStackTaggingImpl?.createTask ?? (() => null)
	}
...
var NgZone = class _NgZone {
  ...
  */
	static isInAngularZone() {
		return (
			typeof Zone !== "undefined" &&
			Zone.current.get("isAngularZone") === true
		)
	}
	/**
...
var ZONE_IS_STABLE_OBSERVABLE = new InjectionToken(
	ngDevMode ? "isStable Observable" : "",
	{
		providedIn: "root",
	...
	validateBegin() {
		if (this.executingCallbacks) {
			throw new RuntimeError(
				102,
				ngDevMode &&
					"A new render operation began before the previous operation ended. Did you trigger change detection from afterRender or afterNextRender?",
			)
		}
	}
	register(callback) {
	 ...
	 */
		...
		ngDevMode &&
			assertGreaterThan(
				this.renderDepth,
				0,
				"renderDepth must be greater than 0",
			)
		this.renderDepth--
...
var ComponentFactory = class extends ComponentFactory$1 {
	...
	create(
		...
		const rootFlags = this.componentDef.signals
			? signalFlags
			: nonSignalFlags
				...
				throw new RuntimeError(
					...
					ngDevMode &&
						`Directives cannot inherit Components. Directive ${stringifyForError(
							definition.type,
						)} is attempting to extend component ${stringifyForError(
							superType,
						)}`,
				)
			}
...
function findHostDirectiveDefs(currentDef, matchedDefs, hostDirectiveDefs) {
	if (currentDef.hostDirectives !== null) {
		for (const hostDirectiveConfig of currentDef.hostDirectives) {
			const hostDirectiveDef = getDirectiveDef(
				hostDirectiveConfig.directive,
			)
			if (typeof ngDevMode === "undefined" || ngDevMode) {
...
function consumeQuotedText(text, quoteCharCode, startIndex, endIndex) {
	...
	throw ngDevMode
		? malformedStyleError(
				text,
				String.fromCharCode(quoteCharCode),
				endIndex,
		  )
		: new Error()
}
...
function stringifyNavigationInstructions(instructions) {
	...
	for (let i = 0; i < instructions.length; i += 2) {
		...
		for (let r = 0; r < repeat2; r++) {
			container.push(
				step === NodeNavigationStep.FirstChild
					? "firstChild"
					: "nextSibling",
			)
		}
	...
	if (Array.isArray(newCollection)) {
		...
		while (liveStartIdx <= liveEndIdx && liveStartIdx <= newEndIdx) {
			...
			if (
				...
				detachedItems.set(
					liveStartKey,
					liveCollection.detach(liveStartIdx),
				)
				liveEndIdx--
			}
		}
...
var _populateDehydratedViewsInLContainer = (lContainer, tNode, hostLView) =>
	false
...
function locateOrCreateAnchorNode(lContainer, hostLView, hostTNode, slotValue) {
	if (
		!_populateDehydratedViewsInLContainer(lContainer, hostTNode, hostLView)
	) {
...
function ɵɵrepeater(collection) {
	...
	try {
		...
		if (metadata.liveCollection === void 0) {
			...
			const itemTemplateTNode = getExistingTNode(
				hostTView,
				containerIndex,
			)
		...
		if (metadata.hasEmptyBlock) {
			...
			if (bindingUpdated(hostLView, bindingIndex, isCollectionEmpty)) {
				...
				const lContainerForEmpty = getLContainer(
					hostLView,
					emptyTemplateIndex,
				)
...
;(function (DeferBlockBehavior2) {
	...
	DeferBlockBehavior2[(DeferBlockBehavior2["Playthrough"] = 1)] =
		"Playthrough"
...
function getTemplateIndexForState(newState, hostLView, tNode) {
	...
	switch (newState) {
		...
		default:
			ngDevMode &&
				throwError2(`Unexpected defer block state: ${newState}`)
			return null
	}
}
			...
			return new IntersectionObserver((entries) => {
				for (const current of entries) {
					if (
						...
						viewportTriggers.has(current.target)
					) {
						ngZone.run(
...
function ɵɵdeferEnableTimerScheduling(
	...
	if (loadingConfigIndex != null) {
		tDetails.loadingBlockConfig = getConstant(
			tViewConsts,
			loadingConfigIndex,
		)
	}
	...
	if (bindingUpdated(lView, bindingIndex, rawValue)) {
		...
		try {
			...
			if (
				value === true &&
				tDetails.loadingState ===
					DeferDependenciesLoadingState.NOT_STARTED
			) {
...
function applyDeferBlockState(
	...
	if (stateTmplIndex !== null) {
		...
		addLViewToLContainer(
			...
			embeddedLView,
			viewIndex,
			shouldAddViewToDom(tNode2, dehydratedView),
		)
...
function triggerResourceLoading(tDetails, lView, tNode) {
	...
	if (ngDevMode) {
		...
		if (deferDependencyInterceptor) {
			dependenciesFn =
				deferDependencyInterceptor.intercept(dependenciesFn)
		}
	}
	if (!dependenciesFn) {
		...
		(results) => {
			...
			for (const result of results) {
				if (result.status === "fulfilled") {
					...
					const directiveDef =
						getComponentDef(dependency) ||
						getDirectiveDef(dependency)
					if (directiveDef) {
...
;(function (LocaleDataIndex2) {
	...
	LocaleDataIndex2[(LocaleDataIndex2["DaysStandalone"] = 4)] =
		"DaysStandalone"
	LocaleDataIndex2[(LocaleDataIndex2["MonthsFormat"] = 5)] = "MonthsFormat"
	LocaleDataIndex2[(LocaleDataIndex2["MonthsStandalone"] = 6)] =
		"MonthsStandalone"
	LocaleDataIndex2[(LocaleDataIndex2["Eras"] = 7)] = "Eras"
	LocaleDataIndex2[(LocaleDataIndex2["FirstDayOfWeek"] = 8)] =
		"FirstDayOfWeek"
...
function getInsertInFrontOfRNodeWithI18n(parentTNode, currentTNode, lView) {
	...
	if (insertBeforeIndex === null) {
		return getInsertInFrontOfRNodeWithNoI18n(
			parentTNode,
			currentTNode,
			lView,
		)
...
function setTNodeInsertBeforeIndex(tNode, index) {
	...
	if (insertBeforeIndex === null) {
		...
		assertEqual(
			Array.isArray(insertBeforeIndex),
			true,
...
function applyCreateOpCodes(
	...
	for (let i = 0; i < createOpCodes.length; i++) {
		...
		if (rNode === null) {
			rNode = lView[index] = isComment
				? renderer.createComment(text)
				: createTextNode(renderer, text)
		}
				...
				case 0:
					...
					if (parentRNode !== null) {
						...
						if (tIcu !== null && typeof tIcu === "object") {
							ngDevMode && assertTIcu(tIcu)
							const caseIndex = getCurrentICUCaseIndex(
								tIcu,
								lView,
							)
							if (caseIndex !== null) {
			...
			switch (opCode) {
				case ICU_MARKER:
					...
					if (lView[commentNodeIndex] === null) {
						...
						ngDevMode && ngDevMode.rendererCreateComment++
						ngDevMode &&
							assertIndexInExpandoRange(lView, commentNodeIndex)
				...
				case ELEMENT_MARKER:
					...
					if (lView[elementNodeIndex] === null) {
						...
						ngDevMode && ngDevMode.rendererCreateElement++
						ngDevMode &&
							assertIndexInExpandoRange(lView, elementNodeIndex)
...
function applyIcuUpdateCase(tView, tIcu, bindingsStartIndex, lView) {
	...
	if (activeCaseIndex !== null) {
		...
		if (activeCaseIndex < 0) {
			activeCaseIndex = lView[tIcu.currentCaseLViewIndex] =
				~activeCaseIndex
			mask = -1
		}
		applyUpdateOpCodes(
...
function loadIcuContainerVisitor() {
	...
	function enterIcu(tIcu, lView) {
		...
		if (currentCase !== null) {
			ngDevMode &&
				assertNumberInRange(currentCase, 0, tIcu.cases.length - 1)
...
function i18nCreateOpCodesToString(opcodes) {
	...
	for (let i = 0; i < createOpCodes.length; i++) {
		...
		const index = opCode >>> I18nCreateOpCode.SHIFT
		lines.push(
			`lView[${index}] = document.${
				isComment ? "createComment" : "createText"
			}(${JSON.stringify(text)});`,
		)
		if (appendNow) {
...
function i18nUpdateOpCodesToString(opcodes) {
	const parser = new OpCodeParser(
		opcodes || (Array.isArray(this) ? this : []),
	)
	let lines = []
	...
	while (parser.hasMore()) {
		...
		while (parser.i < end) {
			...
			if (typeof value === "string") {
				...
				statements.push(
					opCodeText.replace("$$$", "`" + statement + "`") + ";",
				)
				statement = ""
			}
		}
		lines.push(
			`if (mask & 0b${mask.toString(2)}) { ${statements.join(" ")} }`,
		)
	}
	return lines
}
function icuCreateOpCodesToString(opcodes) {
	...
	while (parser.hasMore()) {
		...
		if (value === ICU_MARKER) {
			...
			lastRef = parser.consumeNumber()
			lines.push(
				`lView[${lastRef}] = document.createTextNode("${value}")`,
			)
		} else if (typeof value === "number") {
		...
		if ((i & 1) === 0) {
			...
			for (let j = 0; j < parts.length; j++) {
				...
				if ((j & 1) === 0) {
					const text = part
					ngDevMode &&
						assertString(text, "Parsed ICU part should be string")
					if (text !== "") {
					...
					const icuContainerTNode = createTNodeAndAddOpCode(
						...
						createOpCodes,
						ngDevMode
							? `ICU ${index}:${icuExpression.mainBinding}`
							: "",
						true,
					)
			...
			const index2 =
				HEADER_OFFSET +
				Number.parseInt(value.substring(isClosing ? 2 : 1))
			if (isClosing) {
...
function parseIcuCase(
	...
	const inertBodyElement =
		inertBodyHelper2.getInertBodyElement(unsafeCaseHtml)
	ngDevMode &&
	...
	while (currentNode) {
		...
		switch (currentNode.nodeType) {
			...
			case Node.TEXT_NODE:
				...
				break
			case Node.COMMENT_NODE:
				const isNestedIcu = NESTED_ICU.exec(
...
function i18nPostprocess(message, replacements = {}) {
	...
	if (PP_MULTI_VALUE_PLACEHOLDERS_REGEXP.test(message)) {
		...
		result = result.replace(PP_PLACEHOLDERS_REGEXP, (m, phs, tmpl) => {
			...
			if (!placeholders.length) {
				throw new Error(
					`i18n postprocess: unmatched placeholder - ${content}`,
				)
			}
			const currentTemplateId =
				templateIdsStack[templateIdsStack.length - 1]
			let idx = 0
			...
			const [templateId, isCloseTemplateTag, placeholder] =
				placeholders[idx]
...
function ɵɵtext(index, value = "") {
	...
	const tNode = tView.firstCreatePass
		...
		: tView.data[adjustedIndex]
	const textNative = _locateOrCreateTextNode(
		tView,
		lView,
		tNode,
		value,
	...
	if (flattenValues.some(isForwardRef)) {
		return () =>
			flattenValues
				...
				.map(maybeUnwrapModuleWithProviders)
	} else {
	...
	if (tCleanup && lCleanup) {
		for (let i = 0; i < tCleanup.length; ) {
			...
			if (typeof firstParam === "string") {
				...
				const type =
					typeof useCaptureOrIndx === "boolean" ||
					useCaptureOrIndx >= 0
						? "dom"
						: "output"
				const useCapture =
...
function setClassMetadata(type, decorators, ctorParameters, propDecorators) {
	return noSideEffects(() => {
		...
		if (decorators !== null) {
			if (
				clazz.hasOwnProperty("decorators") &&
				clazz.decorators !== void 0
			) {
...
function ɵɵpureFunction7(
	...
	return bindingUpdated3(lView, bindingIndex + 4, exp5, exp6, exp7) ||
		different
		? updateBinding(
				lView,
				bindingIndex + 7,
		...
		for (let i = 0; i < tQueryMatches.length; i += 2) {
			...
			if (tNodeIdx > 0) {
				...
				for (
					...
					if (
						embeddedLView[DECLARATION_LCONTAINER] ===
						embeddedLView[PARENT]
					) {
						collectQueryResults(
							embeddedLView[TVIEW],
							embeddedLView,
			...
			if (ngModuleDef === null) {
				...
				ngModuleDef = compiler.compileNgModule(
					...
					`ng:///${moduleType.name}/ɵmod.js`,
					{
						type: moduleType,
						bootstrap: flatten(
							ngModule.bootstrap || EMPTY_ARRAY,
						).map(resolveForwardRef),
		...
		get: () => {
			if (ngInjectorDef === null) {
				...
				const meta = {
					...
					imports: [
						(ngModule.imports || EMPTY_ARRAY).map(
							resolveForwardRef,
						),
						(ngModule.exports || EMPTY_ARRAY).map(
							resolveForwardRef,
						),
					],
				}
...
function generateStandaloneInDeclarationsError(type, location2) {
	const prefix = `Unexpected "${stringifyForError(
		type,
...
function verifySemanticsOfNgModuleDef(
	...
	function verifyDeclarationsHaveDefinitions(type) {
		...
		if (!def) {
			errors.push(
				`Unexpected value '${stringifyForError(
					type,
				)}' declared by the module '${stringifyForError(
					moduleType,
	...
	function verifyDirectivesHaveSelector(type) {
		...
		if (!getComponentDef(type) && def && def.selectors.length == 0) {
			errors.push(
				`Directive ${stringifyForError(
					type,
				)} has no selector, please add it!`,
			)
		}
	}
	...
	function verifyExportsAreDeclaredOrReExported(type) {
		...
		if (kind) {
			if (combinedDeclarations.lastIndexOf(type) === -1) {
				errors.push(
					`Can't export ${kind} ${stringifyForError(
						type,
					)} from ${stringifyForError(
						moduleType,
		...
		if (existingModule && existingModule !== moduleType) {
			if (!suppressErrors) {
				const modules2 = [existingModule, moduleType]
					...
					.sort()
				errors.push(
					`Type ${stringifyForError(
						type,
					)} is part of the declarations of 2 modules: ${
						modules2[0]
					} and ${
						modules2[1]
					}! Please consider moving ${stringifyForError(
						type,
						...
						type,
					)} then import that NgModule in ${modules2[0]} and ${
						modules2[1]
					}.`,
				)
			}
		...
		get: () => {
			if (ngComponentDef === null) {
				...
				if (preserveWhitespaces === void 0) {
					if (
						options !== null &&
						options.preserveWhitespaces !== void 0
					) {
				...
				if (encapsulation === void 0) {
					if (
						options !== null &&
						options.defaultEncapsulation !== void 0
					) {
				...
				try {
					...
					if (metadata.standalone) {
						...
						const { directiveDefs, pipeDefs } =
							getStandaloneDefFunctions(type, imports)
						ngComponentDef.directiveDefs = directiveDefs
						ngComponentDef.pipeDefs = pipeDefs
						ngComponentDef.dependencies = () =>
							imports.map(resolveForwardRef)
					}
				} finally {
					compilationDepth--
				}
				...
				for (const rawDep of imports) {
					...
					if (!!getNgModuleDef(dep)) {
						...
						for (const dir of scope.exported.directives) {
							const def =
								getComponentDef(dir) || getDirectiveDef(dir)
							if (def && !seen.has(dir)) {
								seen.add(dir)
...
function shouldAddAbstractDirective(type) {
	const reflect = getReflect()
	if (
		LIFECYCLE_HOOKS.some((hookName) =>
			reflect.hasLifecycleHook(type, hookName),
		)
	) {
		return true
	}
		...
		images.forEach((image) => {
			if (!this.options?.disableImageSizeWarning) {
				for (const image2 of images) {
					if (
						!image2.getAttribute("ng-img") &&
						this.isOversized(image2)
					) {
...
_InitialRenderPendingTasks.ɵfac = function InitialRenderPendingTasks_Factory(
	t,
) {
	return new (t || _InitialRenderPendingTasks)()
}
_InitialRenderPendingTasks.ɵprov = ɵɵdefineInjectable({
...
function handleInstanceCreatedByInjectorEvent(context2, data) {
	...
	const environmentInjector = context2.injector.get(
		EnvironmentInjector,
		null,
		{ optional: true },
	)
			...
			if (instance2 !== null) {
				if (formattedDependency.flags.host) {
					...
					const lookupFromFirstInjector = firstInjector.get(
						...
						null,
						__spreadProps(
							__spreadValues({}, formattedDependency.flags),
							{ optional: true },
						),
					)
		...
		if (injector instanceof NodeInjector) {
			...
			if (firstInjector instanceof NodeInjector) {
				const moduleInjector =
					getModuleInjectorOfNodeInjector(firstInjector)
...
function internalCreateApplication(config2) {
	try {
		...
		const platformInjector =
			createOrReusePlatformInjector(platformProviders)
		...
		return ngZone.run(() => {
			...
			return _callAndReportToErrorHandler(
				exceptionHandler,
				ngZone,
				() => {
					...
					return initStatus.donePromise.then(() => {
						const localeId = envInjector.get(
							LOCALE_ID,
							DEFAULT_LOCALE_ID,
						)
...
function createPlatformFactory(parentPlatformFactory, name, providers = []) {
	...
	return (extraProviders = []) => {
		...
		if (
			!platform ||
			platform.injector.get(ALLOW_MULTIPLE_PLATFORMS, false)
		) {
...
var _ApplicationRef = class _ApplicationRef {
	constructor() {
		...
		this.componentTypes = []
		this.components = []
		this.isStable = inject(InitialRenderPendingTasks).hasPendingTasks.pipe(
			switchMap((hasPendingTasks) =>
				hasPendingTasks ? of(false) : this.zoneIsStable,
			),
	...
	bootstrap(componentOrFactory, rootSelectorOrNode) {
		...
		if (isComponentFactory) {
			...
			componentFactory =
				resolver.resolveComponentFactory(componentOrFactory)
		}
		...
		for (let i = 0; i < bindingIndexes.length; i++) {
			...
			if (metadataParts.length > 1) {
				...
				for (let j = 1; j < metadataParts.length - 1; j++) {
					value +=
						renderStringify(lView[bindingIndex + j - 1]) +
						metadataParts[j + 1]
				}
	...
	return makeEnvironmentProviders([
		...
		},
		{
			...
			useValue: () => {
				if (
					isPlatformBrowser() &&
					inject(IS_HYDRATION_DOM_REUSE_ENABLED)
				) {
...
@angular/core/fesm2022/core.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-W24NLIQJ.js.map
