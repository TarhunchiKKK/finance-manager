...
;(function (NumberSymbol2) {
	...
	NumberSymbol2[(NumberSymbol2["SuperscriptingExponent"] = 7)] =
	...
	_matchCase(value) {
		...
		if (
			...
			console.warn(
				formatRuntimeError(
					2001,
					`As of Angular v17 the NgSwitch directive uses strict equality comparison === instead of == to match different cases. Previously the case value "${stringifyValue(
						value,
					)}" matched switch expression value "${stringifyValue(
						this._ngSwitch,
					)}", but this is no longer the case with the stricter equality check. Your comparison results return different results using === vs. == and you should adjust your ngSwitch expression and / or values to conform with the strict equality requirements.`,
				),
			)
		}
...
function postInitInputChangeError(dir, inputName) {
	...
	return new RuntimeError(
		...
		`${imgDirectiveDetails(
			dir.ngSrc,
		)} \`${inputName}\` was updated after initialization. The NgOptimizedImage directive will not react to this input change. ${reason} To fix this, either switch \`${inputName}\` to a static value or wrap the image element in an *ngIf that is gated on the necessary value.`,
	)
}
...
function assertNonEmptyWidthAndHeight(dir) {
	...
	if (missingAttributes.length > 0) {
		throw new RuntimeError(
			...
			`${imgDirectiveDetails(
				dir.ngSrc,
			)} these required attributes are missing: ${missingAttributes
				.map((attr) => `"${attr}"`)
				.join(
					", ",
...
@angular/common/fesm2022/common.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-OE5OEHZQ.js.map
